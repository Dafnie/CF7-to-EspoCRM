...
add_action( 'wpcf7_before_send_mail', function( $contact_form ) {
    ...
    if ($settings['duplicate'] != "off") {
        $form_value = $_POST[str_replace( 'cf7_', '', $settings['duplicate'] )];
    ...
    if ( $response_body->total == 0 || $settings['duplicate'] == 'off' ) { //Only create if EspoCRM response 0
       ...
       $fields = _fetch_fields( $settings['mapping'], 'parent_' );
    ...
    $fields = _fetch_fields( $settings['mapping'], 'child_' );
    $body = array_merge([
        'parentId' => $parentid,
...
function _fetch_fields( $settings, $entity ) {
